...
public class DataScopeAspect
{
    ...
    // 配置织入点
    @Pointcut("@annotation(com.lingzhi.crm.common.annotation.DataScope)")
        ...
        for (SysRole role : user.getRoles())
        {
            ...
            else if (DATA_SCOPE_CUSTOM.equals(dataScope))
            {
                sqlString.append(StringUtils.format(
                        " OR {}.create_name IN ( SELECT user_name FROM sys_user WHERE dept_id IN ( SELECT dept_id FROM sys_role_dept WHERE role_id = {} ) )", deptAlias,
                        role.getRoleId()));
//                sqlString.append(StringUtils.format(
...
//                        role.getRoleId()));
            ...
            {
                sqlString.append(StringUtils.format(" OR {}.create_name IN ( SELECT user_name FROM sys_user WHERE dept_id = {} ) ", deptAlias, user.getDeptId()));
//                sqlString.append(StringUtils.format(" OR {}.dept_id = {} ", deptAlias, user.getDeptId()));
            }
            else if (DATA_SCOPE_DEPT_AND_CHILD.equals(dataScope))
            {
                sqlString.append(StringUtils.format(
                        " OR {}.create_name IN ( SELECT user_name FROM sys_user WHERE dept_id IN ( SELECT dept_id FROM sys_dept WHERE dept_id = {} or find_in_set( {} , ancestors ) ) )",
